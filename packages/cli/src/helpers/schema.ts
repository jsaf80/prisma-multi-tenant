...
import { getSharedPath, readSchemaFile, writeSchemaFile } from '@prisma4-multi-tenant/shared'
...
export const updateManagementSchemaFile = async (schemaPath?: string): Promise<boolean> => {
  ...
  if (sharedPath == undefined) throw new Error('@prisma4-multi-tenant/shared not found')
  schemaPath = schemaPath || path.join(sharedPath, 'prisma/schema.prisma')

  if (schemaPath == undefined) throw new Error('Management schema not found')

  let managementProvider = process.env['MANAGEMENT_PROVIDER'] || undefined
  if (managementProvider == undefined) throw new Error('.env MANAGEMENT_PROVIDER not found')
