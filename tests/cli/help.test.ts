...
const commands = {
  ...
  eject: 'Eject prisma4-multi-tenant from your application',
...
describe('help', () => {
  ...
  test('command "help"', async () => {
    const ret = await runShell("prisma4-multi-tenant help"); //prettier-ignore

    expect(ret).toEqual(expect.stringContaining(`prisma4-multi-tenant`));
  ...
  for (let [name, description] of Object.entries(commands)) {
    if (name != 'help') {
      test(`command "${name}"`, async () => {
        const ret = await runShell(`prisma4-multi-tenant ${name} --help`);
