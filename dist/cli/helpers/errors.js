...
import { clientManagementPath } from '../../index.js';
export const printError = (error, args) => {
...
const messageHelp = 'Run `prisma4-multi-tenant --help` to learn how to use this tool';
const messageHelpCommand = (name) => `Run \`prisma4-multi-tenant ${name} --help\` to learn how to use this command`;
const messageList = 'Run `prisma4-multi-tenant list` to list all existing tenants';
const missingCommandOrOption = () => {
    console.log(`
  ${chalk.red(`Missing <command|option> argument`)}
...
const unrecognizedCommandOrOption = (args) => {
    console.log(`
  ${chalk.red(`Unrecognized <command|option>: "${args.join(' ')}"`)}
...
const missingArgs = (command) => {
    console.log(`
  ${chalk.red(`Missing one or more arguments for ${chalk.bold(`${command.name}`)}`)}
...
const missingClientManagement = () => {
    console.log(`
  ${chalk.red(`No management Client found`)}

  Run \`prisma4-multi-tenant generate\` to fix this error
  `);
};
const tenantDoesNotExists = (name) => {
    console.log(`
  ${chalk.red(`No tenants exists with the name "${name}"`)}
...
const tenantAlreadyExists = (name) => {
    console.log(`
  ${chalk.red(`A tenant with the name "${name}" already exists`)}
...
const unrecognizedMigrateAction = (args) => {
    console.log(`
  ${chalk.red(`Unrecognized migrate action "${args}"`)}
...
const reservedTenantName = (name) => {
    console.log(`
  ${chalk.red(`You cannot use "${name}" for the name of a tenant`)}
  `);
};
const missingEnv = (name) => {
    console.log(`
  ${chalk.red(`The env variable "${name}" is required but missing`)}
  `);
};
const cannotMigrateSaveManagement = () => {
    console.log(`
  ${chalk.red(`You cannot \`migrate save\` on the management datasource`)}
  `);
};
